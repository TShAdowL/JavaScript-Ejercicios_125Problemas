...
function DatosValidar() {
    let Bandera = true;
        ...
        if((Bandera) && (TxtLado2.value === "")) {
            TxtLado2.value = "0.00";
            Bandera = false;
        }
        if((Bandera) && (TxtLado3.value === "")) {
            TxtLado3.value = "0.00";
            Bandera = false;
        }
...
function AreaCalcular() {
    let Area;
        Area = parseFloat(TxtLado1.value) + parseFloat(TxtLado2.value) + parseFloat(TxtLado3.value);
        Area = Area / 2;
        Area = Area * (Area - parseFloat(TxtLado1.value)) * (Area - parseFloat(TxtLado2.value)) * (Area - parseFloat(TxtLado3.value));
        Area = Math.sqrt(Area);
            Area = Area.toFixed(2);
